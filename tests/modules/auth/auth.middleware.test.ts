...
import { createAuthenticate } from '../../../src/modules/auth/auth.middleware';
...
describe('authenticate middleware', () => {
  it('authenticate attaches the JWT payload to the request', async () => {
  ...
  it('authenticate rejects a missing bearer header', async () => {
    const authenticate = createAuthenticate(fakeAuthService());
  ...
  it('authenticate rejects an invalid token', async () => {
    const authenticate = createAuthenticate(fakeAuthService());
    await assert.rejects(
      () =>
        authenticate(
