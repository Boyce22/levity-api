...
describe('Swagger', () => {
  it('Swagger exposes TypeBox request bodies as OpenAPI requestBody schemas', async () => {
    const app = Fastify();
  ...
  it('Swagger uses JWT bearer authentication and keeps auth endpoints public', async () => {
    const app = Fastify();
  ...
  it('Swagger groups routes by resource tag instead of default', async () => {
    const app = Fastify();
    await app.register(fastifySwagger, swaggerOptions);
