...
import { GenerateInviteInput, UpdateMemberRoleInput } from '../schemas';
import { WorkspaceMemberResponse, WorkspaceInviteResponse } from '../dtos/workspace-response.dto';
...
export class MembersService {
  ...
  async generateInvite(userId: string, workspaceId: string, input: GenerateInviteInput): Promise<WorkspaceInviteResponse> {
  ...
  async updateMemberRole(actorId: string, workspaceId: string, memberId: string, input: UpdateMemberRoleInput): Promise<WorkspaceMemberResponse> {
  ...
  async revokeInvite(userId: string, workspaceId: string, inviteId: string): Promise<void> {
    ...
    if (!invite || invite.workspace_id !== workspaceId) throw new Error('Invite not found');
    if (invite.revoked_at) throw new Error('Invite already revoked');
